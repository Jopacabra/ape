...
rap_max_jet_finder = 100.0  # Maximum rapidity of constituents to consider
...
# Binning
pTmin_vn = 10.0  # don't use zero, since we log space bins.
pTmax_vn = 100.0
num_pt_bins = 5
...
for case in ["v", "m"]:
    ...
    for i, jet in enumerate(analyzed_jets):
        ...
        # v1
        current_v1s = observables.fastjet_intrajetvnish_total(jet=jet, flow=flow, n=1, pt_weighting=pt_weighting)
        v1s.append(np.array(current_v1s, dtype=np.float32))

        # v2
        current_v2s = observables.fastjet_intrajetvnish_total(jet=jet, flow=flow, n=2, pt_weighting=pt_weighting)
        v2s.append(np.array(current_v2s, dtype=np.float32))

        # v3
        current_v3s = observables.fastjet_intrajetvnish_total(jet=jet, flow=flow, n=3, pt_weighting=pt_weighting)
...
if plot_group == "diff":
    ...
    # v1
    axis.plot(E_bin_cents, diff_bin_vals_v1, marker='o', markersize=7, linestyle='-',
              color=color[flow_i], linewidth=2, label=flow + r' $\Delta v_1$', zorder=3)
    axis.fill_between(E_bin_cents, diff_bin_vals_v1 - diff_bin_errs_v1, diff_bin_vals_v1 + diff_bin_errs_v1,
                      color=color[flow_i], alpha=0.25, zorder=2)

    # v2
    axis.plot(E_bin_cents, diff_bin_vals_v2, marker='x', markersize=8, linestyle='--',
              color=color[flow_i+1], linewidth=2, label=flow + r' $\Delta v_2$', zorder=3)
    axis.fill_between(E_bin_cents, diff_bin_vals_v2 - diff_bin_errs_v2, diff_bin_vals_v2 + diff_bin_errs_v2,
                      color=color[flow_i+1], alpha=0.25, zorder=2)

    # v3
    axis.plot(E_bin_cents, diff_bin_vals_v3, marker='^', markersize=7, linestyle=':',
              color=color[flow_i+2], linewidth=2, label=flow + r' $\Delta v_3$', zorder=3)
    axis.fill_between(E_bin_cents, diff_bin_vals_v3 - diff_bin_errs_v3, diff_bin_vals_v3 + diff_bin_errs_v3,
    ...
    # v1 -- AA
    axis.plot(E_bin_cents, bin_vals_v1, marker='o', markersize=7, linestyle='-',
              color=color[flow_i], linewidth=2, label=flow + r' AA $\Delta v_1$', zorder=3)
    axis.fill_between(E_bin_cents, bin_vals_v1 - bin_errs_v1, bin_vals_v1 + bin_errs_v1,
                      color=color[flow_i], alpha=0.25, zorder=2)

    # v2 -- AA
    axis.plot(E_bin_cents, bin_vals_v2, marker='x', markersize=8, linestyle='--',
              color=color[flow_i + 1], linewidth=2, label=flow + r' AA $\Delta v_2$', zorder=3)
    axis.fill_between(E_bin_cents, bin_vals_v2 - bin_errs_v2, bin_vals_v2 + bin_errs_v2,
                      color=color[flow_i + 1], alpha=0.25, zorder=2)

    # v3 -- AA
    axis.plot(E_bin_cents, bin_vals_v3, marker='^', markersize=7, linestyle=':',
              color=color[flow_i + 2], linewidth=2, label=flow + r' AA $\Delta v_3$', zorder=3)
    axis.fill_between(E_bin_cents, bin_vals_v3 - bin_errs_v3, bin_vals_v3 + bin_errs_v3,
                      color=color[flow_i + 2], alpha=0.25, zorder=2)
    ...
    # v1 -- pp
    axis.plot(E_bin_cents, v_bin_vals_v1, marker='o', markersize=7, linestyle='-',
              color=color[flow_i], linewidth=2, label=flow + r' pp $\Delta v_1$', zorder=3)
    axis.fill_between(E_bin_cents, v_bin_vals_v1 - v_bin_errs_v1, v_bin_vals_v1 + v_bin_errs_v1,
                      color=color[flow_i], alpha=0.25, zorder=2)

    # v2 -- pp
    axis.plot(E_bin_cents, v_bin_vals_v2, marker='x', markersize=8, linestyle='--',
              color=color[flow_i + 1], linewidth=2, label=flow + r' pp $\Delta v_2$', zorder=3)
    axis.fill_between(E_bin_cents, v_bin_vals_v2 - v_bin_errs_v2, v_bin_vals_v2 + v_bin_errs_v2,
                      color=color[flow_i + 1], alpha=0.25, zorder=2)

    # v3 -- pp
    axis.plot(E_bin_cents, v_bin_vals_v3, marker='^', markersize=7, linestyle=':',
              color=color[flow_i + 2], linewidth=2, label=flow + r' pp $\Delta v_3$', zorder=3)
    axis.fill_between(E_bin_cents, v_bin_vals_v3 - v_bin_errs_v3, bin_vals_v3 + v_bin_errs_v3,
                      color=color[flow_i + 2], alpha=0.25, zorder=2)
...
fig.savefig(f"intrajet_vn_jetpt.png", dpi=150, bbox_inches='tight')

...
def compute_bin_vals(acos, weights, bin_edges, bin_in_jet_x, analyzed_x=None, delta_phi=1):
    ...
    for j, bin_lo in enumerate(bin_edges[:-1]):
        ...
        if bin_in_jet_x:
            ...
            mask = np.sum(cut, axis=1)  # -> N entries in event in this bin
            w = weights * mask  # N entries * weight of event
            total_w += np.nansum(w)
            val = np.nansum(w)
        vals.append(val)
    return np.array(vals) / (total_w * delta_phi)
...
def bootstrap_ratio_or_diff(
    ...
    m_weight = np.nansum(case_data["m"]["weights"])
    v_weight = np.nansum(case_data["v"]["weights"])
...
plot_group = "ratio"    # "ratio", "diff", "both", "m", or "v"
...
for case in ["v", "m"]:
    ...
    print(f"  Single particle:            {n_single_part}")
    ...
    # --- Compute signed deflection for each particle in each accepted jet relative to axis ---
    acos = [
        [observables.signed_deflection(jet1=WTA_jet, jet2=constituent, dir=deflection_dir)
        for constituent in WTA_jet.constituents()]
        for WTA_jet in analyzed_WTA_jets
    ]  # Ragged shape (N_jets, N_constituents)
    lengths = np.array([len(row) for row in acos])  # numbers of constituents
    mask = np.arange(lengths.max()) < lengths[:, None]  # Get a mask of target shape based on max constituents
    result = np.full(mask.shape, np.nan)  # Initialize all-nans
    result[mask] = np.concatenate(acos)  # Flatten and concatenate
    acos = result  # Overwrite list of lists with nan-filled array
    ...
    if bin_in_jet_x:
        ...
        for j, bin_lo in enumerate(active_bin_edges[:-1]):
            ...
            cut    = (acos > bin_lo) & (acos <= bin_hi)
            mask = np.sum(cut, axis=1)  # -> N entries in event in this bin
            w      = analyzed_weights * mask  # N entries * weight of event
            total_w += np.nansum(w)
        ...
        if case == "v":
            v_bin_vals_aco = np.array(v_bin_vals_aco) / (total_w * delta_phi)
            v_bin_errs_aco = np.array(v_bin_errs_aco) / (total_w * delta_phi)
        else:
            bin_vals_aco = np.array(bin_vals_aco) / (total_w * delta_phi)
            bin_errs_aco = np.array(bin_errs_aco) / (total_w * delta_phi)
...
if plot_group in ("ratio", "diff"):
    ...
    if plot_group == "ratio":
        ...
        print(f"Sum of diffs: {np.sum(derived_vals)}")
